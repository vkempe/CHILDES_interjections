...
french_model_quad <- lmer(PropIntrj ~ SpeakerType * poly(age_c, 3) + session_length_s +
                            (1 | corpus_name/target_child_name),
                          data = french_speakers_data)
summary(french_model_quad)

french_speakers_data <- french_speakers_data %>%
  mutate(SpeakerType = relevel(factor(SpeakerType), ref = "non-CHI"))

french_model_quad_releveled <- lmer(PropIntrj ~ SpeakerType * poly(age_c, 3) + session_length_s +
                                      (1 | corpus_name/target_child_name),
                                    data = french_speakers_data)
summary(french_model_quad_releveled)
```
...
pred <- ggpredict(french_model_quad, terms = c("age_c [all]", "SpeakerType"))
...
french_model_contingency_quad <- lmer(PropIntrj ~ SpeakerType * poly(age_c, 2) + session_length_s +
                                        (1 | corpus_name/target_child_name),
                                      data = french_contingency_data)
summary(french_model_contingency_quad)

french_contingency_data <- french_contingency_data %>%
  mutate(SpeakerType = relevel(factor(SpeakerType), ref = "non-CHI"))

french_model_contingency_quad_releveled <- lmer(PropIntrj ~ SpeakerType * poly(age_c, 2) + session_length_s +
                                                   (1 | corpus_name/target_child_name),
                                                 data = french_contingency_data)
summary(french_model_contingency_quad_releveled)
...
pred <- ggpredict(french_model_contingency_quad, terms = c("age_c [all]", "SpeakerType"))
...
# Rebuild has_co_clean and initial_co_clean from whitelist
japanese_clean <- japanese_raw %>%
  mutate(
    words = str_split(gloss, " "),
    tags  = str_split(part_of_speech, " "),
    co_tokens = map2(words, tags, ~ {
      idx <- which(str_detect(.y, "^co(:|$)"))
      consolidate_japanese(.x[idx])
    }),
    has_co_clean = map_lgl(co_tokens, ~ any(.x %in% japanese_whitelist)),
    first_clean_token = map_chr(words, tags, .f = function(w, t) {
      idx <- which(str_detect(t, "^co(:|$)"))
      if (length(idx) == 0) return(NA_character_)
      consolidated <- consolidate_japanese(w[idx])
      match <- which(consolidated %in% japanese_whitelist)
      if (length(match) > 0) consolidated[match[1]] else NA_character_
    }),
    initial_co_clean = map2_lgl(words, tags, ~ {
      if (length(.y) == 0) return(FALSE)
      if (!str_detect(.y[1], "^co(:|$)")) return(FALSE)
      consolidate_japanese(.x[1]) %in% japanese_whitelist
    })
  )
...
# Contingency data (non-CHI utterance following CHI utterance)
japanese_contingent <- japanese_clean %>%
  group_by(transcript_id) %>%
  arrange(utterance_order) %>%
  mutate(
    prev_speaker = lag(SpeakerType),
    is_contingent = SpeakerType == "Non-CHI" & prev_speaker == "CHI"
  ) %>%
  ungroup() %>%
  filter(is_contingent) %>%
  group_by(transcript_id, corpus_name, target_child_name,
           age_months) %>%
  summarise(
    n_contingent = n(),
    n_initial_co = sum(initial_co_clean),
    prop_initial_co = n_initial_co / n_contingent,
    .groups = "drop"
  )
